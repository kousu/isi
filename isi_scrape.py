...
class ISIResponse(requests.Response):
    """
    Wrap an requests.Response to translate ISI's frustratingly non-standard
    non-HTTP-error error messages into the python-requests framework
    """
...
class ISISession(requests.Session):
    ...
    def generalSearch(self, *fields, timespan=None, editions=["SCI", "SSCI", "AHCI", "ISTP", "ISSHP"], sort='LC.D;PY.A;LD.D;SO.A;VL.D;PG.A;AU.A'):
        ...
        assert search_mode == 'GeneralSearch'
        
        return ISIQuery(self, search_mode, qid, r.url, count, estimated)
    ...
    def outlinks(self, document):
        ...
        assert search_mode == 'CitedRefList'
    ...
    def inlinks(self, document):
        ...
        assert search_mode == 'CitingArticles'
...
class ISIQuery:
    ...
    def bulk_inlinks(self, loops=True):
        ...
        citationreport_qid = soup.find("input", {"name": "qid"})['value'] #<-- thank you lazy coders
        ...
        assert search_mode == params["search_mode"]
        ...
        return ISIQuery(self._session, search_mode, qid, r.url, count, estimated)
